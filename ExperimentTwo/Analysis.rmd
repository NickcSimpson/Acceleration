...
```{r plot_average_pscv, echo=FALSE}
...
```{r plot_pscvs, echo=FALSE, cache=TRUE}
plot_horizontal_pscvs = bias %>%
  mutate(gradient = if_else(ACCELERATING-DECELERATING>0, 'negative', 'positive')) %>%
...
ggplot(d = df_const, aes(x=pos, y=prop_acc_resp, colour=gradient)) +
  geom_line(aes(x=x, group = subject_id)) +
  geom_point(aes(x=x, group = subject_id)) +
  stat_summary(
    geom = "linerange",
    fun.data = mean_sdl, 
    fun.args = list(mult = 1),
