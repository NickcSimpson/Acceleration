...
*Hypothesis 1 (ACCELERATION PERCEPTION FOR ACCELERATING VS. DECELERATING DIRECTIONS)*: We will test the null hypothesis that horizontal acceleration perception is similar for accelerating and decelerating directions. For each participant, the proportion of 'accelerating' responses will be calculated for each acceleration value within the range ± 3.6 m/s^2^ and then averaged, separately for the accelerating and decelerating direction. A paired-samples t-test will be performed comparing the proportion of 'accelerating' responses for accelerating vs. decelerating directions.

*Hypothesis 2 (PSCV FOR ACCELERATING VS. DECELERATING)*: We will test the null hypothesis that the point of constant subjective velocity (PSCV) is similar for accelerating and decelerating directions. The PSCV will be estimated using the quickpsy function of the quickpsy R package with acceleration/deceleration values in the range ± 3.6 m/s^2^ as the explanatory variable, and participant and motion direction as grouping factors [@R-quickpsy]. Psychometric curves with a PSCV outside of the tested range (± 8 m/s^2^) will be excluded from analysis. A single PSCV will be calculated for each motion direction by taking the mean of the PSCVs for each screen duration. A paired-samples t-test  will be performed comparing the the PSCV for the accelerating and decelerating directions.
...
```
Using the effect size from the psychometric curve fitting analysis in the previous preregistered experiment (pre-registration: "Transient and stable predictions affect motion perception, Exp. 1"), a sample size of `r sample_size_needed` is necessary for a power of 0.95. Using the effect size from the bias analysis in the previous preregistered experiment (pre-registration: "Transient and stable predictions affect motion perception, Exp. 1"), a sample size of `r bias_sample_size_needed` is necessary for a power of 0.95. As the effect sizes are similar for both analyses and the psychometric curve analysis has stricter exclusion criteria, we base the power analysis of off the bias analysis. Therefore, collecting a sample of `r bias_sample_size_needed` participants should provide sufficient power to observe the effect of interest.

# Analysis

```{r load_data, echo=FALSE}
data_frame <- read.csv("../Results/expTwoResults.txt");
...
df = data_frame %>% 
  ...
  mutate(numerical_correct = if_else(correct=='true', 1, if_else(acceleration == 0, NaN, 0))) %>%
  mutate(recodeKey = if_else(key_response == 'a', 1, if_else(key_response == 'd', 0, NaN))) %>%
  mutate(recodeDir = if_else((direction == 'RIGHT' & right_bias == 'Positive') | (direction == 'LEFT' & left_bias == 'Positive'), 'ACCELERATING', 'DECELERATING')) %>%
  group_by(subject_id) %>%
  mutate(average_accuracy = mean(numerical_correct, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(average_accuracy >= 0.75) %>%
...
Number of participant data sets: `r number_of_original_data_sets`.
Number of included data sets: `r number_of_final_data_sets`.

## Hypothesis 1 (ACCELERATION PERCEPTION FOR ACC VS. DEC DIRECTIONS)
```{r check_bias_horizontal, echo=FALSE, cache=TRUE, fig.cap="Participant bias to report accelerating for the two conditions. Individual participants are shown in faint colours with the colour reflecting the sign of the gradient of the line. A positive gradient (blue) means that the participant responded accelerating more often for the decelerating condition, which implies a suppression effect. A negative gradient (red) means that the participant responded accelerating more often for the accelerating condition, which implies an enhancement effect. The mean across participants is shown in black."}
...
plot_bias = bias %>%
  mutate(gradient = if_else(ACCELERATING-DECELERATING>0, 'negative', 'positive')) %>%
...
## Hypothesis 2 (PSCV FOR ACC VS. DEC)
...
averageFit = quickpsy(d = df %>% filter(subject_id %in% horizontal_pscvs$subject_id), x=acceleration, k=recodeKey, grouping = .(recodeDir, screen_duration), bootstrap = "none");

averageFitOverSD = quickpsy(d = df %>% filter(subject_id %in% horizontal_pscvs$subject_id), x=acceleration, k=recodeKey, grouping = .(recodeDir), bootstrap = "none");

averageFullFit = fullFit$curves %>%
  filter(subject_id %in% horizontal_pscvs$subject_id) %>%
  group_by(recodeDir, screen_duration, x) %>%
  reframe(y = mean(y))
```

The PSCV for 'accelerating' objects is insignificantly more positive than the PSCV for 'decelerating' objects (`r apa_print(horizontal_pscv)$statistic`).

### Plotting all participants

```{r plot_all_pscv, echo=FALSE, fig.cap="Psychometric curves for all participants after removing those with a absolute PSCV greater than 8. The vertical lines show the PSCV for each participant."}
plotcurves(fullFit) + xlim(-4,4) + theme(legend.position="none")
```

### Plotting a psychometric curve generated from treating all responses from all included subjects as one with screen durations separate

```{r plot_one_pscv, echo=FALSE, fig.cap="Psychometric curves fit to the data when all subjects' responses are treated as one."}
plotcurves(averageFit) + xlim(-4,4)
```

### Plotting a psychometric curve generated from treating all responses from all included subjects as one

```{r plot_av_pscv, echo=FALSE, fig.cap="Psychometric curves fit to the data when all subjects' responses are treated as one averaged over screen duration."}
plotcurves(averageFitOverSD) + xlim(-4,4)
```

### Plotting the average value for each subject's psychometric curve for each x value (different x values for screen duration so cannot average)

```{r plot_average_pscv, echo=FALSE, fig.cap="Taking the mean y value for each participant's fitted psychometric curve for every x value in figure [@ref(fig:plot_all_pscv)]."}
ggplot(averageFullFit, aes(x=x, y=y, colour = recodeDir)) + geom_line() + facet_grid(cols=vars(screen_duration))
```

### Plotting the PSCVs for each participant averaged over screen duration

```{r plot_pscvs, echo=FALSE, cache=TRUE, fig.cap="Participant PSCVs extracted from figure [@ref(fig:plot_all_pscv)]. The colour of the individual data corresponds to the gradient of the line. A positive gradient (blue) means that "}
plot_horizontal_pscvs = horizontal_pscvs %>%
  mutate(gradient = if_else(ACCELERATING-DECELERATING>0, 'negative', 'positive')) %>%
  pivot_longer(cols=c('ACCELERATING', 'DECELERATING'), names_to = 'recodeDir', values_to = 'thre') %>%
  mutate(x = if_else(recodeDir == 'ACCELERATING', 0, 1))

if(horizontal_pscv$p.value<0.001){
  annotation = '***';
} else if(horizontal_pscv$p.value<0.01){
  annotation = '**';
} else if(horizontal_pscv$p.value<0.05){
  annotation = '*';
} else {
  annotation = 'NS';
}

ggplot(plot_horizontal_pscvs, aes(x=x, y=thre, colour = gradient)) +
  geom_line(aes(group=subject_id), alpha = 0.4) +
  geom_point(aes(group=subject_id), alpha = 0.4) +
  stat_summary(
    geom = "linerange",
  ...
  annotate("text", x=0.5, y=1.1, label = annotation) +
  ggtitle('Participant PSCV, averaged over screen duration.') +
  xlab('Condition') +
  ylab('PSCV') +
  scale_x_continuous(breaks=c(0, 1), labels=c("Accelerating","Decelerating"))
```

## Looking only at constant velocity trials

Using the trials with constant velocity, we find the proportion of accelerating responses in the 'accelerating' and 'decelerating' directions. A tendency to report constant velocity trials as accelerating indicate that there is a bias to perceive acceleration. A tendency to report constant velocity trials as decelerating indicate that there is a bias to perceive deceleration.

```{r const_vel, echo=FALSE}
...
People respond that a constant velocity object is accelerating more often in the 'decelerating' direction than the 'accelerating' direction (`r apa_print(const_stat)$statistic`).

## Power Comparison

Take the power from each study and compare.
```{r power_comparison, echo=FALSE}
exp_one_bias_power = bias_effect_size;
exp_one_pscv_power = effect_size;
exp_two_bias_power = cohensD(bias$ACCELERATING, bias$DECELERATING, method = "paired")
exp_two_pscv_power = cohensD(horizontal_pscvs$ACCELERATING, horizontal_pscvs$DECELERATING, method = "paired")
```
For the bias analysis, experiment one has an effect size of `r exp_one_bias_power` and experiment two has an effect size of `r exp_two_bias_power`. For the PSCV analysis, experiment one has an effect size of `r exp_one_pscv_power` and experiment two has an effect size of `r exp_two_pscv_power`.
...
```{r bayes_across_time, echo=FALSE, fig.cap="Bayes factor calculated for each participant in order of data collection."}
