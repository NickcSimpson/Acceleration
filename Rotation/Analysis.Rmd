...
data_frame <- read.csv("../Results/nick_test.txt");
...
df = data_frame %>% 
  ...
  ungroup() %>%
  #filter(average_accuracy >= 0.75) %>% #TODO: add this line back in
...
## Hypothesis 1 (ACCELERATION PERCEPTION FOR UPWARD VS. DOWNWARD MOTION)
```{r check_bias_vertical, echo=FALSE, cache=TRUE}

bias = df %>% 
  group_by(subject_id, relative_motion, acceleration, screen_duration) %>% 
  summarise(prop_acc=mean(recodeKey)) %>%
  group_by(subject_id, relative_motion, screen_duration) %>% 
  summarise(screen_bias=mean(prop_acc)) %>% 
  group_by(subject_id, relative_motion) %>% 
  summarise(bias=mean(screen_bias)) %>% 
  spread(relative_motion, bias)

vertical_bias = t.test(bias$UP, bias$DOWN, paired=TRUE)

vertical_bias_bayes = ttestBF(bias$UP, bias$DOWN, paired=TRUE, alternative = 'two.sided', rscale=0.37)
```

People have a bias to report XXXX (`r apa_print(vertical_bias)$statistic`).
...
plot_vertical_bias = bias %>%
  ...
  pivot_longer(cols = c('DOWN', 'UP'), names_to = 'relative_motion', values_to = 'bias') %>%
  mutate(x = if_else(relative_motion == 'DOWN', 0, 1))
...
## Hypothesis 2 (ACCELERATION PERCEPTION FOR PSEUDO-UPWARD VS. PSEUDO-DOWNWARD DIRECTIONS)
```{r check_bias_horizontal, echo=FALSE, cache=TRUE}
horizontal_bias = t.test(bias$PSEUDO_UP, bias$PSEUDO_DOWN, paired='TRUE')
horizontal_bias_bayes = ttestBF(bias$PSEUDO_UP, bias$PSEUDO_DOWN, paired=TRUE, alternative = 'two.sided', rscale=0.37)
```

People have a bias to report XXXX (`r apa_print(horizontal_bias)$statistic`).
...
plot_horizontal_bias = bias %>%
  mutate(gradient = if_else(PSEUDO_DOWN-PSEUDO_UP==0, 'neutral',if_else(PSEUDO_DOWN-PSEUDO_UP>0, 'negative', 'positive'))) %>%
  pivot_longer(cols = c('PSEUDO_DOWN', 'PSEUDO_UP'), names_to = 'relative_motion', values_to = 'bias') %>%
  mutate(x = if_else(relative_motion == 'PSEUDO_DOWN', 0, 1))
...
ggplot(d = plot_horizontal_bias, aes(x=x, y=bias, colour=gradient)) +
  geom_line(aes(group=subject_id), alpha = 0.4) +
  geom_point(aes(group=subject_id), alpha = 0.4) +
  stat_summary(
    geom = "linerange",
  ...
  ylab('Bias') +
  ggtitle('Bias to report accelerating.') +
  scale_x_continuous(breaks=c(0, 1), labels=c("PSEUDO-DOWN","PSEUDO-UP"))
```

## Hypothesis 3 (PSCV FOR UPWARD VS. DOWNWARD)
```{r check_pscv_vertical, echo=FALSE, cache=TRUE}
fullFit <- quickpsy(d = df, x=acceleration, k=recodeKey, grouping = .(subject_id, relative_motion,screen_duration), bootstrap = "none");
...
pscvs = fullFit$thresholds %>%
  ...
  select(subject_id, relative_motion, mean_pscv) %>%
  pivot_wider(names_from = relative_motion, values_from = mean_pscv)

vertical_pscvs = pscvs %>% 
  filter(abs(UP)<=4.8 & abs(DOWN)<=4.8)

vertical_pscv = t.test(vertical_pscvs$UP, vertical_pscvs$DOWN, paired='TRUE')
vertical_pscv_bayes = ttestBF(vertical_pscvs$UP, vertical_pscvs$DOWN, paired=TRUE, alternative='two.sided', rscale=0.37)
```

The PSCV for downward moving objects is XXXX (`r apa_print(vertical_pscv)$statistic`).

```{r plot_mean_pscv, echo=FALSE, fig.cap="Psychometric curves showing the horizontal and vertical biases."}
fullAverageFit <- quickpsy(d = df, x=acceleration, k=recodeKey, grouping = .(relative_motion,screen_duration), bootstrap = "none");
plotcurves(fullAverageFit) + xlim(-4,4)
```

## Hypothesis 4 (PSCV FOR PSEUDO-UPWARD VS. PSEUDO-DOWNWARD)
```{r check_pscv_horizontal, echo=FALSE, cache=TRUE}
horizontal_pscvs = pscvs %>% 
  filter(abs(PSEUDO_DOWN)<=4.8 & abs(PSEUDO_UP)<=4.8)

horizontal_pscv = t.test(horizontal_pscvs$PSEUDO_DOWN, horizontal_pscvs$PSEUDO_UP, paired='TRUE')
horizontal_pscv_bayes = ttestBF(horizontal_pscvs$PSEUDO_DOWN, horizontal_pscvs$PSEUDO_UP, paired=TRUE, alternative='two.sided', rscale=0.37)
```
The PSCV for XXXX (`r apa_print(horizontal_pscv)$statistic`).
...
hyp1_bayes_factors = list();
hyp2_bayes_factors = list();
for(i in 2:nrow(this_bias)){
  this_set = this_bias %>% filter(SID<=i);
  hyp1_bayes_factors = c(hyp1_bayes_factors, as.numeric(as.vector(ttestBF(this_set$DOWN, this_set$UP, paired=TRUE, alternative='two.sided', rscale=0.37))));
  hyp2_bayes_factors = c(hyp2_bayes_factors, as.numeric(as.vector(ttestBF(this_set$PSEUDO_DOWN, this_set$PSEUDO_UP, paired=TRUE, alternative='two.sided', rscale=0.37))));
}

pscvs_minus_ver_outliners = pscvs %>% filter(abs(DOWN)<=4.8 & abs(UP)<=4.8)
pscvs_minus_hor_outliners = pscvs %>% filter(abs(PSEUDO_DOWN)<=4.8 & abs(PSEUDO_UP)<=4.8)

this_pscv = cbind(pscvs_minus_ver_outliners, data.frame(SID = 1:nrow(pscvs_minus_ver_outliners)));
hyp3_bayes_factors = list();
for(i in 2:nrow(this_pscv)){
  this_set = this_pscv %>% filter(SID<=i);
  hyp3_bayes_factors = c(hyp3_bayes_factors, as.numeric(as.vector(ttestBF(this_set$DOWN, this_set$UP, paired=TRUE, alternative='two.sided', rscale=0.37))));
}

this_pscv = cbind(pscvs_minus_hor_outliners, data.frame(SID = 1:nrow(pscvs_minus_hor_outliners)));
hyp4_bayes_factors = list();
for(i in 2:nrow(this_pscv)){
  this_set = this_pscv %>% filter(SID<=i);
  hyp4_bayes_factors = c(hyp4_bayes_factors, as.numeric(as.vector(ttestBF(this_set$PSEUDO_DOWN, this_set$PSEUDO_UP, paired=TRUE, alternative='two.sided', rscale=0.37))));
}

max_length = max(length(hyp1_bayes_factors), length(hyp2_bayes_factors), length(hyp3_bayes_factors), length(hyp4_bayes_factors))
hyp1_bayes_factors = c(hyp1_bayes_factors, rep(NA, length(hyp1_bayes_factors)-max_length));
hyp2_bayes_factors = c(hyp2_bayes_factors, rep(NA, length(hyp2_bayes_factors)-max_length));
hyp3_bayes_factors = c(hyp3_bayes_factors, rep(NA, length(hyp3_bayes_factors)-max_length));
hyp4_bayes_factors = c(hyp4_bayes_factors, rep(NA, length(hyp4_bayes_factors)-max_length));

bayes_factor_df = data.frame(bf_hyp1 = as.numeric(hyp1_bayes_factors),
                             bf_hyp2 = as.numeric(hyp2_bayes_factors),
                             bf_hyp3 = as.numeric(hyp3_bayes_factors),
                             bf_hyp4 = as.numeric(hyp4_bayes_factors),
                             participant_count = 2:max_length) %>%
  pivot_longer(cols = c('bf_hyp1', 'bf_hyp2', 'bf_hyp3', 'bf_hyp4'), names_prefix = 'bf_', names_to = 'hypothesis', values_to = 'calc')

ggplot(bayes_factor_df, aes(x=participant_count, y=calc, colour = hypothesis)) +
  geom_point() +
  ...
  ylab('Bayes Factor') +
  ggtitle('Bayes Factor Progression With Increased Participants.') + 
  scale_colour_discrete(name = "Hypothesis Number", labels = c("1", "2", "3", "4"))
```
