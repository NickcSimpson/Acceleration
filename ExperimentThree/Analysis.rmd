...
```{r load_data}
data_frame <- read.csv("../Results/expThreeResults.txt");
number_of_original_data_sets = length(unique(data_frame$subject_id))-2;
...
horizontal_bias_bayes = ttestBF(bias$ACCELERATING, bias$DECELERATING, paired=TRUE, alternative = 'two.sided', rscale=0.14)
```
People have a bias to report the 'accelerating' objects as having a less positive acceleration than the 'decelerating' objects (`r apa_print(horizontal_bias)$statistic`, `r apa_print(horizontal_bias_bayes)$statistic`).

Mean accelerating bias =`r mean(bias$ACCELERATING)`.
Mean decelerating bias =`r mean(bias$DECELERATING)`.

## Hypothesis Two

```{r check_pscv_horizontal, echo=FALSE, cache=TRUE}
fullFit <- quickpsy(d = df, x=acceleration, k=recodeKey, grouping = .(subject_id, condition, screen_duration), bootstrap = "none");
...
horizontal_pscv_bayes = ttestBF(horizontal_pscvs$ACCELERATING, horizontal_pscvs$DECELERATING, paired=TRUE, alternative = 'two.sided', rscale=0.14)
```
People have a bias to report the 'accelerating' objects as having a less positive acceleration than the 'decelerating' objects (`r apa_print(horizontal_pscv)$statistic`, `r apa_print(horizontal_pscv_bayes)$statistic`).

Mean accelerating pscv =`r mean(horizontal_pscvs$ACCELERATING)`.
Mean decelerating pscv =`r mean(horizontal_pscvs$DECELERATING)`.
...
for(i in 2:nrow(this_pscv)){
  ...
  bayes_factors_pscv = c(bayes_factors_pscv, as.numeric(as.vector(ttestBF(this_set$ACCELERATING, this_set$DECELERATING, paired=TRUE, alternative='two.sided', rscale = 0.14))));
}
...
ggplot(bayes_factor_df, aes(x=participant_count, y=bias, colour = test_type)) +
  geom_point() +
  geom_segment(aes(x = 0, xend = nrow(bayes_factor_df)/2, y = 3, yend = 3)) +
  geom_segment(aes(x = 0, xend = nrow(bayes_factor_df)/2, y = 1/3, yend = 1/3)) +
  scale_y_continuous(trans='log10')
```

```{r plot_trial_order, echo=FALSE, fig.cap="Trial order across the experiment."}

## Plot the order in which trials are seen

trial_order = data_frame %>% 
  filter(trial_type == "moving-image") %>% #Select rows that are trials and not setup etc
  mutate(acceleration = as.numeric(acceleration)) %>%
  mutate(numerical_correct = if_else(correct=='true', 1, if_else(acceleration == 0, NaN, 0))) %>%
  mutate(recodeKey = if_else(key_response == 'a', 1, if_else(key_response == 'd', 0, NaN))) %>%
...
#ggplot(trial_order, aes(x = trial_index, y = subject_id, shape = recodeColour, colour = extreme, size = acceleration)) + geom_jitter(height = 0.25, width = 0)
```
