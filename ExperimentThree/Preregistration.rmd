---
title             : "Transient and stable predictions affect motion perception, Exp. 3 - Pre-registration document"
shorttitle        : "Transient and stable predictions affect motion perception, Exp. 3 - Pre-registration document"
...
In a previous experiment, we found perceptual repulsion effects for expectations about the acceleration of objects on the horizontal axis. An expectation was induced such that objects would accelerate more often in one direction (accelerating direction) and decelerate more often in the other (decelerating direction). Objects in the accelerating direction were perceived as accelerating less than objects in the decelerating direction. The observed repulsion from expectation contrasts with many attractive perceptual expectation effects. Due to the expectation depending on the direction of travel of the object, it is possible that adaptation-like effects could create the repulsive bias. Therefore, the current study aims to eliminate any adaptation-like effects by inducing the expectation through the colour of the stimulus as opposed to the direction of motion.
...
```{r design1, echo=FALSE, fig.cap="**Layout.** A light grey square is constantly shown on the screen. On each trial, a coloured circle travels across the square. Participants must decide if the circle is accelerating or decelerating (here orange is accelerating and purple is decelerating) and can respond once it has left the square.", out.width = '40%'}
knitr::include_graphics(c("Orange.png", "Purple.png"))
#ggdraw() + 
 # draw_image("Orange.png", width = 0.5) + 
  #draw_image("Purple.png", width = 0.5, x = 0.5)
```

To avoid participants using the amount of time the circle is on the screen as an indication of acceleration or deceleration, the circle remains on the screen for one of two screen durations, 1000 or 1500 ms. For each of the two screen durations, 11 equally spaced acceleration values are chosen from the range of ± $\frac{2 \cdot square \: length}{screen \: duration ^ 2}$. A positive skew is given to one colour (Fig. \@ref(fig:design2), bottom row, hereafter accelerating colour) and a negative skew to the other one (Fig. \@ref(fig:design2), top row, hereafter decelerating colour), randomly assigned per participant. The skewed distributions have a mean acceleration of ± 1.74 m/s^2^ and also include three extreme trials with maximal acceleration/deceleration. These extreme values are presented in the first block on the 1st, 3rd, and 5th trial of the respective colour to induce a strong expectation early in the experiment. 

```{r design2, echo=FALSE, fig.cap="**Acceleration distributions.** The distribution of acceleration and deceleration values presented for each coloured circle are skewed in order to induce an expectation of acceleration in one colour (bottom panel) and of deceleration in the other (top panel). These skewed distributions have a mean acceleration of ± 1.74 m/s^2^. Long trials (green) remain on the screen for 1500 ms and short trials (blue) for 1000 ms. Extreme trials (red) have maximal acceleration/deceleration and are used to induce a strong expectation within the first five trials for that colour. All extreme trials have a screen duration of 1000 ms.", out.width = '110%'}
...
This study is designed to eliminate adaptation-like effects causing a repulsive bias in acceleration perception in the horizontal plane. The data will be analysed in the same way as for the previous experiment (pre-registration: "Transient and stable predictions affect motion perception, Exp. 2").

*Hypothesis 1 (ACCELERATION PERCEPTION FOR ACCELERATING VS. DECELERATING DIRECTIONS)*: We will test the null hypothesis that horizontal acceleration perception is similar for accelerating and decelerating coloured objects. For each participant, the proportion of 'accelerating' responses will be calculated for each acceleration/deceleration value within the range ± 3.6 m/s^2^ and then averaged, separately for the screen durations and the accelerating/decelerating colour. The bias for each screen duration will be averaged to give a single value for each colour for each participant. A paired-samples t-test will be performed comparing the proportion of 'accelerating' responses for accelerating vs. decelerating colours.

*Hypothesis 2 (PSCV FOR ACCELERATING VS. DECELERATING)*: We will test the null hypothesis that the point of subjective constant velocity (PSCV) is similar for accelerating and decelerating colours. The PSCV will be estimated using the quickpsy function of the quickpsy R package with acceleration/deceleration values in the range ± 3.6 m/s^2^ as the explanatory variable, and participant, screen duration and colour as grouping factors [@R-quickpsy]. Psychometric curves with a PSCV outside of the tested range (± 8 m/s^2^) will be excluded from analysis. A single PSCV will be calculated for each colour by taking the mean of the PSCVs for each screen duration. A paired-samples t-test  will be performed comparing the PSCV for the accelerating and decelerating colours.
